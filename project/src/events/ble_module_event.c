...
#include "ble_module_event.h"
#include "common_module_event.h"

static char *get_evt_type_str(enum ble_module_event_type type)
{
	switch (type) {
	case BLE_EVT_CONNECTED:
		return "BLE_EVT_CONNECTED";
	case BLE_EVT_PAIRING:
		return "BLE_EVT_PAIRING";
...
{
	const struct ble_module_event *event = cast_ble_module_event(aeh);
...
COMMON_APP_EVENT_INFO_DEFINE(ble_module_event,
			 profile_event);

#endif /* CONFIG_NRF_PROFILER */

COMMON_APP_EVENT_TYPE_DEFINE(ble_module_event,
			 log_event,
			 &ble_module_event_info,
			 APP_EVENT_FLAGS_CREATE(
				IF_ENABLED(CONFIG_BLE_EVENTS_LOG,
