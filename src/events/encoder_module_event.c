...
#include "encoder_module_event.h"
...
static char *get_evt_type_str(enum encoder_module_event_type type)
{
	...
	case ENCODER_EVT_SHUTDOWN_READY:
		return "ENCODER_EVT_SHUTDOWN_READY";
	case ENCODER_EVT_ERROR:
		return "ENCODER_EVT_ERROR";
	default:
...
{
	const struct encoder_module_event *event = cast_encoder_module_event(aeh);

	if (event->type == ENCODER_EVT_ERROR) {
		APP_EVENT_MANAGER_LOG(aeh, "%s - Error code %d",
				get_evt_type_str(event->type), event->data.err);
	} else if (event->type == ENCODER_EVT_DATA_READY) {
		APP_EVENT_MANAGER_LOG(aeh, "%s - (ENCODER_A, ENCODER_B)[deg/s] = (%f, %f)",
...
COMMON_APP_EVENT_INFO_DEFINE(encoder_module_event,
			 profile_event);

#endif /* CONFIG_NRF_PROFILER */

COMMON_APP_EVENT_TYPE_DEFINE(encoder_module_event,
			 log_event,
			 &encoder_module_event_info,
			 APP_EVENT_FLAGS_CREATE(
				IF_ENABLED(CONFIG_ENCODER_EVENTS_LOG,
